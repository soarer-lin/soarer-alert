...
DOCS_DIR = aiops-docs
HEALTH_CHECK_API = $(SERVER_URL)/vector-store/health
DOCKER_COMPOSE_FILE = docker-compose.yml
POSTGRES_CONTAINER = soarer-alert-postgres
...
# 默认目标：显示帮助信息
help:
	@echo "$(GREEN)SoarerAlertAgent Makefile$(NC)"
	@echo ""
	@echo "Windows PowerShell 推荐使用:"
	@echo "  .\\scripts\\dev-infra.ps1"
	@echo "  .\\scripts\\dev-app.ps1"
	@echo ""
	@echo "可用命令："
	@echo "  $(YELLOW)make init$(NC)    - 🚀 一键初始化（启动Docker → 启动服务 → 上传文档）"
	@echo "  $(YELLOW)make up$(NC)      - 启动 Docker Compose（PostgreSQL + pgvector、Redis、RustFS、Prometheus）"
...
init:
	@echo "$(GREEN)🚀 开始一键初始化 SoarerAlertAgent...$(NC)"
	@echo ""
	...
	@echo "   PostgreSQL + pgvector: localhost:15432"
	@echo ""
...
start:
	...
	@if curl -s -f $(HEALTH_CHECK_API) > /dev/null 2>&1; then \
		...
		nohup mvn -s maven-settings.local.xml spring-boot:run > server.log 2>&1 & \
		echo $$! > server.pid; \
...
check:
	...
	@if curl -s -f $(HEALTH_CHECK_API) > /dev/null 2>&1; then \
		...
		echo "$(YELLOW)请先启动项目: mvn -s maven-settings.local.xml spring-boot:run$(NC)"; \
		exit 1; \
	fi
...
up:
	...
	@if docker ps --format '{{.Names}}' | grep -q "^$(POSTGRES_CONTAINER)$$"; then \
		...
		if docker ps --format '{{.Names}}' | grep -q "^$(POSTGRES_CONTAINER)$$"; then \
			...
			echo "   RustFS API: http://localhost:19000"; \
		else \
...
down:
	...
	@if docker ps --format '{{.Names}}' | grep -q "^$(POSTGRES_CONTAINER)$$"; then \
		...
		echo "$(YELLOW)⚠️  没有运行中的 PostgreSQL 容器$(NC)"; \
	fi
...
status:
	...
	@if docker ps -a --format '{{.Names}}' | grep -q "^$(POSTGRES_CONTAINER)$$"; then \
		...
		echo "$(YELLOW)⚠️  没有找到 SoarerAlertAgent 相关容器$(NC)"; \
