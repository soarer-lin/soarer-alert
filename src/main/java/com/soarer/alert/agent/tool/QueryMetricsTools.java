...
public class QueryMetricsTools {
    ...
    @Tool(description = "Query active alerts from Prometheus alerting system. " +
            ...
            "Labels such as instance, hostname, job, and environment are preserved so multi-host alerts remain distinguishable. " +
        ...
        try {
            ...
            if (mockEnabled) {
                ...
                for (PrometheusAlert alert : result.getData().getAlerts()) {
                    ...
                    simplified.setLabels(alert.getLabels() == null ? new LinkedHashMap<>() : new LinkedHashMap<>(alert.getLabels()));
    ...
    private List<SimplifiedAlert> buildMockAlerts() {
        ...
        slowAlert.setLabels(Map.of(
                "instance", "pod-user-service-6d9e8f1f2-n4o5p",
