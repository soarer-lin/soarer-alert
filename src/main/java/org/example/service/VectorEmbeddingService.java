...
/**
 * 向量嵌入服务，使用阿里云 DashScope Text Embedding API。
 */
@Service
public class VectorEmbeddingService {
    ...
    public void init() {
        if (!isApiKeyConfigured()) {
            logger.warn("DashScope API Key 未配置，知识库向量检索将在调用时不可用");
    ...
    private void requireConfiguredApiKey() {
        if (!isApiKeyConfigured()) {
            throw new IllegalStateException(
                    "请设置环境变量 DASHSCOPE_API_KEY 或在 application.yml 中配置正确的 API Key"
            );
        }
    }

    private void ensureGlobalApiKey() {
        if (Constants.apiKey == null || Constants.apiKey.trim().isEmpty()) {
            Constants.apiKey = apiKey;
        }
    }
}
